...
* 3.4   ms   01/23/17 Added kprintf statement in main function to
 ...
 */
#define EEPROM_ADDRESS	0x54	 /* 0x54 as an 8 bit number */
...
int main(void)
{
	...
	if (Status != XST_SUCCESS) {
		kprintf("IIC lowlevel eeprom Example Failed\r\n");
		return XST_FAILURE;
	}

	kprintf("Successfully ran IIC lowlevel eeprom Example\r\n");
	return XST_SUCCESS;
}
...
int IicLowLevelEeprom()
{
	...
	Status =  MuxInit(); //配置IIC复用芯片到EEPROM
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
int ReadWriteVerify(AddressType Address)
{
	...
	kprintf("0x%x: Write to the EEPROM success.\n", Address);
	...
	kprintf("0x%x: Read from the EEPROM success.\n", Address);
	...
	kprintf("0x%x: Verify the data read against the data written success.\n", Address);
	...
	kprintf("0x%x: Read each byte one at a time and verify success.\n\n", Address);
...
int MuxInit(void)
{
	...
	Status = XIic_Send(IIC_BASE_ADDRESS, IIC_MUX_ADDRESS, &channel, 1, XIIC_STOP);

	if (Status != 1) {
		return XST_FAILURE;
	}

	kprintf("IIC Mux select EEPROM\n\n");
	return XST_SUCCESS;
}
